    ...
    def __getitem__(self, index):
      ...
      #새롭게 added
        ...
        if rr > 0.8:
            ...
            #img, gt = CutPaste(img, gt, 0.1, 0.15, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img = Cutout(img, random.uniform(0.05, 0.1), True, np.random.uniform(self.trainsize),
            #             np.random.uniform(self.trainsize))
        elif rr > 0.6:
            img, gt = CutPaste(img, gt, 0.1, 0.2, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            ...
            #              np.random.uniform(self.trainsize))
            img, gt = CutPaste(img, gt, 0.1, 0.2,  np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img, gt = CutPaste(img, gt, 0.2, 0.3, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
        elif rr > 0.4:
            img, gt = CutPaste(img, gt, 0.1, 0.2,  np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img = Cutout(img, random.uniform(0.05, 0.1), True, np.random.uniform(self.trainsize),
            #              np.random.uniform(self.trainsize))
...
class UnlabeledPolypDataset(data.Dataset):
    """
    dataloader for unlabeled polyp segmentation tasks
    """
    ...
    def __init__(self, image_root, trainsize, augmentations):
        ...
        print(self.augmentations)
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        #elf.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.png')]
        self.images = sorted(self.images)
        #self.gts = sorted(self.gts)
        #self.filter_files()
        self.size = len(self.images)
    ...
    def __getitem__(self, index):
        ...
        if rr > 0.8:
            ...
            #img = CutPasteImage(img, random.uniform(0.05, 0.1), True, np.random.uniform(self.trainsize),
            #             np.random.uniform(self.trainsize))
            img, _ = CutPaste(img, None, 0.1, 0.2, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            img, _ = CutPaste(img, None, 0.05, 0.1,  np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img, gt = CutPaste(img, gt, 0.1, 0.15, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img = Cutout(img, random.uniform(0.05, 0.1), True, np.random.uniform(self.trainsize),
            #             np.random.uniform(self.trainsize))
        elif rr > 0.6:
            ...
            img, _ = CutPaste(img, None, 0.2, 0.3, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img = Cutout(img, random.uniform(0.05, 0.1), True, np.random.uniform(self.trainsize), np.random.uniform(self.trainsize))
            #img = CutPasteImage(img, random.uniform(0.05, 0.1), True, np.random.uniform(self.trainsize),
            #            np.random.uniform(self.trainsize))
            #img = _Cutout(img, random.uniform(0.02, 0.05), True, np.random.uniform(self.trainsize),
...
def get_loader(image_root, gt_root, labeled, batchsize, trainsize, shuffle=True, num_workers=4, pin_memory=True,
               augmentation=False, droplast = False):
    if labeled == True:
        dataset = PolypDataset(image_root, gt_root, trainsize, augmentation)
    else:
        dataset = UnlabeledPolypDataset(image_root, trainsize, augmentation)

    data_loader = data.DataLoader(dataset=dataset,
                                  batch_size=batchsize,
                                  shuffle=shuffle,
                                  num_workers=num_workers,
                                  pin_memory=pin_memory, drop_last=droplast)
    return data_loader
