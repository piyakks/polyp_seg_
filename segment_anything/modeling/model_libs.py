...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=4):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1) #channel값은 똑같은 1*1값 출력
        self.max_pool = nn.AdaptiveMaxPool2d(1) #channel값은 똑같은 1*1값 출력

        self.sharedMLP = nn.Sequential(
            nn.Conv2d(in_planes, in_planes // ratio, 1, bias=False), nn.ReLU(),
            nn.Conv2d(in_planes // ratio, in_planes, 1, bias=False))#channel을 /4로 줄였다가 다시 늘림
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        avgout = self.sharedMLP(self.avg_pool(x))
        maxout = self.sharedMLP(self.max_pool(x))
        return self.sigmoid(avgout + maxout)#in_plane channel*1*1출력
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)#7*7 커널로
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        avgout = torch.mean(x, dim=1, keepdim=True)#b*1*h*w
        maxout, _ = torch.max(x, dim=1, keepdim=True)#b*1*h*w
        x = torch.cat([avgout, maxout], dim=1)#b*2*h*w->b*1*h*w
        x = self.conv(x)
...
class DoubleAttentionModule(nn.Module):
    def __init__(self, in_chs, out_chs):
        ...
        self.db = DecoderBlock(in_chs, out_chs)#outchs=32로 설정되어있음 3*3 kernel로 두번  
        self.ca = ChannelAttention(out_chs)# out_chs*1*1출력 #channel에 정보값 
        self.sa = SpatialAttention()#1*h*w # 위치에 대한 정보값 

    def forward(self, x):
        out_db = self.db(x)#(outchs=32)*h*w
        
        out_ca = self.ca(out_db) * out_db#channel에 대한 가중치 
        
        out_sa = self.sa(out_db) * out_db #위치에 대한 가중치

        return out_ca + out_sa# 둘을 더해줌 
...
class BoundaryEnhancementModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x_h = self.horizontal_conv(x)# 이미지 수평에 대한 정보
        x_h = Upsample(x_h, (w, h))# 352,346 -> 352,352
        x_v = self.vertical_conv(x)# 수직에 대한 정보 
        x_v = Upsample(x_v, (w, h))# 346,352 -> 352,352
        x_arr = x.cpu().numpy().transpose((0, 2, 3, 1)).astype(np.uint8)# 넘파이로 바꿔줌
        canny = np.zeros((bs, 1, w, h))
        
        for i in range(bs):
            canny[i] = cv2.Canny(x_arr[i], 10, 100)# 10이상 이면 엣지 , 100부터 엣지
        canny = torch.from_numpy(canny).to(device).float()

        h_canny = torch.cat((x_h, canny), dim=1)#canny와 이미지 수평 정보 concat
        v_canny = torch.cat((x_v, canny), dim=1)#canny와 이미지 수직 정보 concat 
        h_v_canny = torch.cat((self.conv1x1_h(h_canny), self.conv1x1_v(v_canny)), dim=1)# 각 이미지 정보를 2,w,h ->8,w,h 이거 두개 ->16,w,h 
        h_v_canny_out = self.conv_out(h_v_canny)#16,w,h->32,w,h
...
class _AtrousSpatialPyramidPoolingModule(nn.Module):
    ...
    def forward(self, x, s_edge):  # x: bs,192,22,22 fused_semantic_boundary_s:bs, 1, 352, 352
        ...
        img_features = self.img_pooling(x)#192,1,1
        img_features = self.img_conv(img_features)#64,1,1
        ...
        edge_features = F.interpolate(s_edge, x_size[2:],
                                      mode='bilinear', align_corners=True)#1,h/4*w/4
        edge_features = self.edge_conv(edge_features)#64,h/4*w/4
        out = torch.cat((out, edge_features), 1)#64,h/4*w/4

        for f in self.features:
            y = f(x)
            
            out = torch.cat((out, y), 1)#64씩 4개 256
        return out #384*h/4*w/4
...
class RFB_modified(nn.Module):
    def __init__(self, in_chl, out_chl):
        ...
        self.branch1 = nn.Sequential(
            ...
            BasicConv2d(out_chl, out_chl, 3, padding=3, dilation=3)#결과적으로 image size는 변함 없음 
        )
    ...
    def forward(self, x):
        x0 = self.branch0(x)#image size는 동일 out_channel값에 따라 x0,x1,x2,x3값 모두 out_channel값 동일 
        x1 = self.branch1(x)#
        x2 = self.branch2(x)#
        x3 = self.branch3(x)#
        x = self.relu(self.conv_cat(torch.cat((x0, x1, x2, x3), 1)) + self.conv_res(x))
        #out_channel*4의 채널을 out_channel로 바꿔줌 결과적으로 out_channel*352*352
        # conv_res는 out_channel*352*352  
        return x
...
class Dense_Aggregation_input_5(nn.Module):
    def __init__(self, chl):
        ...
        self.conv_d1_1=Conv3x3(chl * 4, chl * 5)
        self.conv_d2 = nn.Conv2d(chl * 5, 1, 1)
        self.pool=nn.AvgPool2d(2,2)
        

    def forward(self, x0, x1, x2, x3):  # x0:bs,32,22,22 x1:bs,32,44,44 x2:bs,32,88,88 x3:bs,32,196,196 x4:bs,ch,*2
        x0_1 = x0  # bs,32,22,22
        ...
        #x4_1 = self.conv4_0(self.up2(self.up2(self.up2(x0)))) * self.conv3_1(
        ...
        #x4_5= self.pool(x4_5)# 다시 /4로

        x=self.conv_d1_1(x3_4)#64*4=256 w/4
        #x = self.conv_d1(x4_5)#320
        #x = self.conv_d2(x)
...
class Dense_Aggregation_input_3(nn.Module):
    ...
    def forward(self, x2, x3, x4):
        x4_1 = x4
        x4_2 = self.conv4_1(self.up2(x4)) * x3#f5를 upsampling으로 *2해줌 그리고 f4과 곱 size=(64*h/8*w/8)
        x4_3 = self.conv4_2(self.up2(self.up2(x4))) * self.conv3_1(self.up2(x3)) * x2# f5 upsampling*2 , f4 upsampling 한 것과 곱을 conv 후 f3과 곱 size=64*h/4*w/4

        x43_cat = self.conv_cat_4_3(torch.cat((x4_2, self.conv4_3(self.up2(x4_1))), 1))# x4_2와  f5와 concat size= 128*h/8*w/8
        x32_cat = self.conv_cat_3_2(torch.cat((x4_3, self.conv3_2(self.up2(x43_cat))), 1))#x4_3과 (x43_cat)을 upsampling한 걸 upsampling 후 concat 192*h/4*w/4
        

        out = self.conv_out(x32_cat)#192*h/4*w/4


        return out
class Inception_ResNet_A(nn.Module):
    def __init__(self, in_chn, scale=1):
        super(Inception_ResNet_A, self).__init__()
        out_chn = in_chn // 8
        self.conv1 = nn.Conv2d(in_chn, out_chn, 1, 1, 0) # in, out, kernel, stride, padding
        self.conv2 = nn.Sequential(self.conv1,
                                   BasicConv2d(out_chn, out_chn, 3, 1, 1))
        self.conv3 = nn.Sequential(self.conv1,
                                   BasicConv2d(out_chn, out_chn+(out_chn//2), 3, 1, 1),
                                   BasicConv2d(out_chn+(out_chn//2), out_chn+out_chn, 3, 1, 1))
        self.convf = nn.Sequential(nn.Conv2d(out_chn*4, in_chn//scale, 1, 1, 0), #
                                   nn.BatchNorm2d(in_chn//scale)) #
...
class Aggregation4(nn.Module):
    # dense aggregation, it can be replaced by other aggregation previous, such as DSS, amulet, and so on.
    # used after MSF
    def __init__(self, channel, out_ch):
        super(Aggregation4, self).__init__()
        self.relu = nn.ReLU(True)
        ...
        self.conv_concat2 = Inception_ResNet_A(2 * channel)
        self.conv_concat3 = Inception_ResNet_A(3 * channel)
        self.conv_concat4 = Inception_ResNet_A(4 * channel)

        self.conv4_1 = Inception_ResNet_A(4 * channel, scale=2) #
        self.conv4_2 = Inception_ResNet_A(2 * channel, scale=2) #
        # self.conv5 = nn.Conv2d(channel, out_ch, 1) # output_channels = 12
        self.conv5 = nn.Conv2d(4*channel, out_ch, 1) # output_channels = 12 내가 바꾼거
    def forward(self, x1, x2, x3, x4):
        ...
        x2_2 = torch.cat((x2_1, self.conv_upsample5(self.upsample(x1_1))), 1)
        x2_2 = self.conv_concat2(x2_2)

        x3_2 = torch.cat((x3_1, self.conv_upsample6(self.upsample(x2_2))), 1)
        x3_2 = self.conv_concat3(x3_2)

        x4_2 = torch.cat((x4_1, self.conv_upsample7(self.upsample(x3_2))), 1)
        x4_2 = self.conv_concat4(x4_2)
        x4_2= self.conv5(x4_2) #원랜 이 문장 없음 
        # x = self.conv4_1(self.upsample(x4_2))
        # x = self.conv4_2(self.upsample(x))
        # x = self.conv5(x)
